...
.phip_palette_map <- function(levels_vec, palette = phip_palette) {
  if (length(levels_vec) == 0L) return(stats::setNames(character(0), character(0)))
...
.get_palette <- function(n) {
  ...
  if (is.null(pal)) return(scales::hue_pal()(n))
...
.hex2rgb <- function(hex) {
  if (is.null(hex) || is.na(hex) || !nzchar(hex)) return(c(NA_real_, NA_real_, NA_real_))
  as.numeric(grDevices::col2rgb(hex))/255
}
.rgb2hex <- function(x) {
  if (length(x) < 3 || anyNA(x)) return("#B3B3B3")
  x <- pmin(1, pmax(0, as.numeric(x[1:3])))
  grDevices::rgb(x[1], x[2], x[3])
}
.mix_cols <- function(hex_a, hex_b, t) {
  if (is.na(hex_a) || is.na(hex_b) || is.na(t)) return("#B3B3B3")
  a <- .hex2rgb(hex_a); b <- .hex2rgb(hex_b)
  if (anyNA(a) || anyNA(b)) return("#B3B3B3")
  .rgb2hex((1 - t) * a + t * b)
}
...
.pick_axes <- function(df, axes=c(1,2)) {
  ...
  for (stem in c("CAP","PCoA","PC","MDS")) {
    ...
    if (all(cols %in% names(df))) return(cols)
  }
  .ph_abort(sprintf(
...
.centroids_df <- function(df, axes_cols, by) {
  if (is.null(by) || !(by %in% names(df))) return(NULL)
  ...
  if (shade %in% c("auto","time")) {
    if (has_tc) {
      ...
      norm_t <- ave(df[[time_cont]], groups, FUN = function(z) {
        ...
        if (!is.finite(diff(rng)) || diff(rng) == 0) return(rep(0.5, length(z)))
        (z - rng[1]) / diff(rng)
      })
...
.make_point_fills <- function(df, base_map, group_col = "group",
  ...
  if (mode == "time" && has_tc) {
    ...
    t_norm <- ave(df[[time_cont]], grp, FUN = function(z) {
      z <- as.numeric(z); r <- range(z, na.rm = TRUE)
      if (!is.finite(diff(r)) || diff(r) == 0) return(rep(0.5, length(z)))
      (z - r[1]) / diff(r)
    })
...
.blend_hex <- function(c1, c2 = "#FFFFFF", w = 0.5) {
  ...
  r1 <- grDevices::col2rgb(c1)/255; r2 <- grDevices::col2rgb(c2)/255
  out <- (1 - w) * r1 + w * r2
