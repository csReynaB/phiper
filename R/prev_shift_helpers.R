...
.one_contrast_internal <- function(dat_uv, weight_mode, stat_mode, prev_strat, B, seed,
  ...
  if (identical(prev_strat, "none") &&
    ...
    if (identical(design, "paired")) {
      ...
      if (!identical(hits_g1$peptide_id, hits_g2$peptide_id)) {
        hits_g2 <- hits_g2 %>%
          dplyr::semi_join(hits_g1, by = "peptide_id") %>%
          dplyr::arrange(peptide_id)
        hits_g1 <- hits_g1 %>%
          dplyr::semi_join(hits_g2, by = "peptide_id") %>%
          dplyr::arrange(peptide_id)
      }
      if (nrow(hits_g1) == 0L) {
        return(NULL)
  ...
  if (design == "paired") {
    ...
    .T_from_assignment <- function(flip_vec) {
      x1 <- numeric(length(Jmap))
      n1p <- numeric(length(Jmap))
      x2 <- numeric(length(Jmap))
      n2p <- numeric(length(Jmap))
      names(x1) <- names(Jmap)

      fset <- as.logical(flip_vec)
      names(fset) <- subj
      ...
      .combine_T_internal(
        p1p[keep_p], p2p[keep_p], n1p[keep_p], n2p[keep_p],
        winsor_z, weight_mode, stat_mode, prev_strat
      )$T_obs
    }
    ...
    .T_from_split <- function(idx1) {
      lab <- rep(NA_character_, length(all_ids))
      names(lab) <- all_ids
      lab[idx1] <- g1
      ...
      n1p <- tmp[[paste0("n_", g1)]][keep_p]
      n2p <- tmp[[paste0("n_", g2)]][keep_p]
      .combine_T_internal(
        p1p[keep_p], p2p[keep_p], n1p, n2p,
        winsor_z, weight_mode, stat_mode, prev_strat
      )$T_obs
    }
