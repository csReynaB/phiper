  ...
  .ph_with_timing(
    ...
    expr = {
      ...
      base_present <- dl |>
        ...
        dplyr::select(.data$subject_id, .data$sample_id, .data$peptide_id,
                      !!grp, !!tcol,
      ...
      if (identical(time_mode, "reference")) {
        ...
        stab <- switch(similarity,
                       ...
                       kulczynski = stab |>
                         dplyr::mutate(
                           ...
                           similarity = dplyr::coalesce(0.5 * (t1 + t2),
        ...
        tryCatch(
          ...
          error = function(e) {
            .ph_abort(
              ...
              step     = "dplyr::compute()",
              bullets  = c(sprintf("table: %s", table_name),
                           sprintf("error: %s", conditionMessage(e)))
            )
          }
        )
        ...
        L0 <- sets |>
          dplyr::rename(subject_id1 = .data$subject_id, group1 = !!grp, time1 = !!tcol,
                        peptide_id = .data$peptide_id, sample_id1 = .data$sample_id)
        R0 <- sets |>
          dplyr::rename(subject_id2 = .data$subject_id, group2 = !!grp, time2 = !!tcol,
          ...
          if (kind == "same") {
            ...
            inters <- pairs_scoped |>
              dplyr::count(.data$time, .data$group1, .data$subject_id1,
                           .data$group2, .data$subject_id2, name = "a")

            pairs <- inters |>
              dplyr::left_join(
                sizes |> dplyr::rename(group1 = !!grp, time = !!tcol,
                                       subject_id1 = .data$subject_id, n1 = .data$n),
                by = c("time","group1","subject_id1")
              ) |>
              dplyr::left_join(
                sizes |> dplyr::rename(group2 = !!grp, time = !!tcol,
            ...
            pairs <- switch(similarity,
                            ...
                            phi = {
                              ...
                              pairs |>
                                dplyr::left_join(
                                  sid_map |> dplyr::rename(group1 = .data$group, subject_id1 = .data$subject_id,
                                                           sample_id1 = .data$sample_id),
                                  by = c("time","group1","subject_id1")
                                ) |>
                                dplyr::left_join(
                                  sid_map |> dplyr::rename(group2 = .data$group, subject_id2 = .data$subject_id,
            ...
            inters <- pairs_scoped |>
              dplyr::count(.data$time1, .data$group1, .data$subject_id1,
                           .data$time2, .data$group2, .data$subject_id2, name = "a")

            pairs <- inters |>
              dplyr::left_join(
                sizes |> dplyr::rename(group1 = !!grp, time1 = !!tcol,
                                       subject_id1 = .data$subject_id, n1 = .data$n),
                by = c("time1","group1","subject_id1")
              ) |>
              dplyr::left_join(
                sizes |> dplyr::rename(group2 = !!grp, time2 = !!tcol,
                                       subject_id2 = .data$subject_id, n2 = .data$n),
            ...
            pairs <- switch(similarity,
                            ...
                            phi = {
                              ...
                              pairs |>
                                dplyr::left_join(
                                  sid_map |> dplyr::rename(group1 = .data$group, time1 = .data$time,
                                                           subject_id1 = .data$subject_id, sample_id1 = .data$sample_id),
                                  by = c("time1","group1","subject_id1")
                                ) |>
                                dplyr::left_join(
                                  sid_map |> dplyr::rename(group2 = .data$group, time2 = .data$time,
...
plot_similarity_heatmap <- function(sim_tbl,
  ...
  req_new    <- c("time_left","group_left","subject_left",
                  "time_right","group_right","subject_right",
                  "similarity","pairing")
  req_legacy <- c("timepoint","group_left","subject_left",
  ...
  if (using_legacy) {
    ...
    sim_tbl <- sim_tbl |>
      dplyr::mutate(time_left  = .data$timepoint,
                    time_right = .data$timepoint,
                    pairing    = "same") |>
      dplyr::select(.data$time_left,  .data$group_left,  .data$subject_left,
  ...
  if (dyads != "no" && !has_dy) {
    .ph_warn(
      ...
      bullets  = c("Expected: dyad_left, dyad_right.",
  ...
  if (is.null(groups)) {
    .chk_cond(length(groups_all) != 1L,
              ...
              bullets = c(sprintf("unique groups: %s",
    ...
    .chk_cond(is.null(times) || length(times) != 2L,
              ...
              step = "input resolution",
              bullets = sprintf("available times: %s",
                                paste(head(times_all, 20), collapse = ", "))
    )
    left_group  <- groups_all[1]; right_group <- groups_all[1]
    left_time   <- times[1];      right_time  <- times[2]
  } else {
    ...
    left_group  <- groups[1]; right_group <- groups[2]
    .chk_cond(is.null(times) || length(times) != 2L,
  ...
  if (dyads != "no" && has_dy) {
    ...
    Llist <- split(unique(df[, c("dyad_left","subject_left")])$subject_left,
                   unique(df[, c("dyad_left","subject_left")])$dyad_left)
    Rlist <- split(unique(df[, c("dyad_right","subject_right")])$subject_right,
                   unique(df[, c("dyad_right","subject_right")])$dyad_right)

    .build_map <- function(subj_vec, K) {
      subj_vec <- sort(unique(subj_vec))
      if (length(subj_vec) == 0L) return(data.frame(subject = character(0), idx = integer(0), axis = character(0)))
    ...
    y_levels <- left_map  |> dplyr::arrange(.data$dyad, .data$idx)  |> dplyr::pull(.data$axis) |> unique()
    x_levels <- right_map |> dplyr::arrange(.data$dyad, .data$idx)  |> dplyr::pull(.data$axis) |> unique()
