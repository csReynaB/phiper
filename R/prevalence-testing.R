...
ph_prevalence_compare <- function(x,
  ...
  .ph_with_timing(headline = "prevalence_compare",
                  step     = NULL,
                  bullets  = NULL,
                  expr = {
    ...
    tryCatch({
      ...
      chk::chk_number(prevalence_threshold); chk::chk_true(prevalence_threshold >= 0)
    ...
    df_ranked_long <- df_ranked |>
      tidyr::pivot_longer(cols = tidyselect::all_of(available_ranks),
    ...
    build_group_view <- function(tbl) {
      if (!interaction) {
        ...
        c1 <- combine_cols[1]; c2 <- combine_cols[2]
    ...
    fpr_n <- features_per_rank |> dplyr::summarise(n = dplyr::n()) |> dplyr::collect()
    if (fpr_n$n == 0L) {
    ...
    base_grid <- group_sizes |>
      ...
      dplyr::inner_join(
        features_per_rank |> dplyr::mutate(.dummy = 1L), by = ".dummy"
      ) |>
